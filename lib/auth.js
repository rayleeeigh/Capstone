...
export const signup = async (cred) => {
  const query = 'INSERT INTO accounts (username, password, first_name, last_name, birthdate, type, status, created_by, updated_by, updated_at) VALUES (?, ?, ?, ?, ?, ?, 1, ?, 1, ?)';
  const results = await executeQuery({ query: query, values: [cred.username, cred.password, cred.firstname, cred.lastname, cred.birthdate, cred.type, cred.created_by, cred.updated_at] });
  return results;
};
